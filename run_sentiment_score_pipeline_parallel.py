...
df_daterange = pd.DataFrame(pd.date_range('2019-03-15','2022-07-15',freq='M'), columns=['Monthend'])
df_daterange['update_date'] = df_daterange['Monthend'].apply(lambda x: str(x)[:7]+'-15')
datelist = list(df_daterange['update_date'].unique())

version = 'v0.1.0'

def main(data_date, version):
    # update_data = True
    ...
    generate_model_output.generate_model_output(data_date=data_date, version=version)
    print('-----------------------------------------------')
    print('Finish generate model output')
    print('-----------------------------------------------')

for data_date in datelist:
    print('-----------------------------------------------')
    print(data_date, ' start to product.')
    main(data_date, version)
